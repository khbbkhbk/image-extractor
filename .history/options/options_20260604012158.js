...
async function init() {
  ...
  $("#namingMode").addEventListener("change", syncNamingTemplateVisibility);
}
...
function bindToForm() {
  ...
  $("#namingMode").value = config.download.singleUseSourceFilename === false ? "template" : "source";
  $("#antiHotlinkEnabled").checked = config.download.antiHotlink?.enabled !== false;
  syncNamingTemplateVisibility();
}

function syncNamingTemplateVisibility() {
  const isTemplate = $("#namingMode").value === "template";
  const fields = $("#templateFields");
  fields.dataset.disabled = isTemplate ? "0" : "1";
  $("#filenameTemplate").disabled = !isTemplate;
}
...
function readForm() {
  return {
    ...
    download: {
      ...
      singleUseSourceFilename: $("#namingMode").value === "source",
      antiHotlink: {

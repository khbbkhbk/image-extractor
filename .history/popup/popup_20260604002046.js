...
async function applyScanPayload(payload, tab = {}, { preserveFilters = false, incremental = false } = {}) {
  ...
  if (preserveFilters) {
    ...
    if (!previousSelected.size) state.images.forEach((image) => state.selected.add(image.id));
...
async function downloadSelected() {
  ...
  await startDownload(images, "正在创建下载任务").catch((error) => setMessage(error.message));
}
...
async function retryRateLimited() {
  ...
  stopAutoRetry();
  state.autoRetryStopped = false;
  await startDownload(state.rateLimitedImages, "正在重下 429 图片").catch((error) => setMessage(error.message));
}
...
function getStableKey(image) {
  const url = String(image?.url || image?.originalUrl || "").trim();
  return url ? url : null;
}

...
async function startDownload(images, label, { autoRetry = false } = {}) {
  ...
  try {
    ...
    const doneText = `下载完成：成功 ${response.result.count}/${response.result.requestedCount || images.length}${failedText}${rateLimitedText}`;
    showToast(doneText);
    setMessage(doneText);
...
function setDownloadState(isDownloading) {
  ...
  if (abortBtn) abortBtn.disabled = !state.isDownloading;
}

async function abortCurrentDownload() {
  if (!state.isDownloading || !state.activeSessionId) {

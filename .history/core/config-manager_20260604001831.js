...
export class ConfigManager {
  ...
  migrate(saved) {
    ...
    if (savedVersion < 7) {
      const profileName = migrated.profileName || DEFAULT_CONFIG.profileName;
      const profile = CONFIG_PROFILES[profileName] || CONFIG_PROFILES.comic;
      migrated.scan = {
        ...(migrated.scan || {}),
        includeBackground: profileName === "comic"
          ? Boolean(profile?.scan?.includeBackground ?? DEFAULT_CONFIG.scan.includeBackground)
          : (typeof migrated.scan?.includeBackground === "boolean"
            ? migrated.scan.includeBackground
            : Boolean(profile?.scan?.includeBackground ?? DEFAULT_CONFIG.scan.includeBackground))
      };
    }
    return migrated;

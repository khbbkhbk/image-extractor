...
async function downloadOne(image, context, index, config) {
  ...
  const resolvedUrl = extractFirstHttpUrl(image?.editedUrl || image?.url || "");
  if (!resolvedUrl) throw new Error("图片地址为空");
  const shouldMaterialize = shouldConvert || config.antiHotlink?.enabled || resolvedUrl.startsWith("blob:") || resolvedUrl.startsWith("data:");
  ...
  if (shouldMaterialize) {
    assertNotAborted(config.sessionId);
    const sourceBlob = await fetchImageBlob(resolvedUrl, config);
    assertNotAborted(config.sessionId);

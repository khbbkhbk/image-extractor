...
- 单图下载默认使用源文件名，并按配置的请求间隔启动请求；遇到 HTTP 429 或连接超时类错误会统计数量，并支持自动或手动重下这批图片。
...
- 预览图通过后台代理加载并返回 data URL，避免弹窗直接请求远端图片导致 CORS、防盗链失败或 UI 抖动。
- 侧边栏/弹窗打开期间，切换标签页或当前页面加载完成会自动重新扫描并刷新预览。
- 下载失败采用三档处理：`429`、超时、网络波动、浏览器 `interrupted` 中的临时失败会在 1 秒后自动重试；`403`、`404`、命名异常、格式转换失败仅提供手动重试；明显不可恢复的参数错误只给出文本提示，不进入重试队列。
